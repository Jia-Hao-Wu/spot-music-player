...
import { getAlbumDetail, getTrackStream } from "@/api";
	...
	return (
		<div className="flex flex-1 flex-col bg-background overflow-y-auto">
			<div className="sticky top-0 z-10 flex items-center gap-3 px-4 py-3 bg-background/80 backdrop-blur">
				...
				<span className="text-sm text-foreground font-medium truncate">{album.title}</span>
			</div>

			<div className="flex flex-col items-center px-4 pb-4">
				<img
					src={artworkUrl(album.cover, ARTWORK_SIZES.large)}
					className="w-48 h-48 rounded-md object-contain"
				/>
				<div className="mt-3 text-center">
					<div className="text-base text-foreground font-medium">{album.title}</div>
					<div className="text-xs text-muted">{album.artist?.name}</div>
					{album.releaseDate && (
						<div className="text-xs text-muted mt-1">{album.releaseDate.slice(0, 4)}</div>
					)}
				</div>
			</div>

			<div className="flex flex-col">
				{items.map(({ item: track }, index) => (
					<div
						key={track.id}
						...
						onClick={async () => {
							enQueue({
								...
								album: album.title,
								artwork: artworkUrl(album.cover, ARTWORK_SIZES.thumbnail),
